...
export default function Email(props: TextProps) {
  const {
    isAudits,
    block,
    onEdit,
  ...
  const {email, name} = block
  const {placeholder, read_only: readOnly, value = ''} = email || {}
    ...
    <Content {...props}>
      ...
      <BodyContainer>
        {!readOnly && !isAudits ? (
          ...
          <StyledLink href={`mailto:${renderValue}`}>{renderValue}</StyledLink>
        )}
      </BodyContainer>
    </Content>
  )
